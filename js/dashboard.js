document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('bio-text').textContent = profile.bio || t('dashboard.noBioYet');
  ...
  async function renderActivity() {
    ...
    recent.forEach((r) => {
      ...
      row.className =
        'bg-surface-container-low p-space-md rounded-lg border border-outline-variant flex items-center justify-between gap-space-sm';
      row.innerHTML = `
        <div>
  ...
  document.addEventListener('zen:languagechange', renderActivity);
});
